...
use crate::{
    ...
    Request, RequestVoteResponse, RequestVoteResponseReason, Response, Result, State, Timeout,
};
    ...
    P: Response,
{
        ...
        P: Send + 'static,
    {
        ...
        tokio::spawn(timeouts(retry.clone(), election.clone(), async move {
            ...
            let peers_len = session.node.inner.state.read().await.get_membership().len();
    ...
    async fn get_unack_peers(&self) -> Result<Option<HashSet<NodeId>>> {
        ...
        let peers = self.node.inner.state.read().await;
        let peers = peers
            .get_membership()
            .keys()
